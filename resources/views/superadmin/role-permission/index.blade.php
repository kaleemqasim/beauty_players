...
@foreach($roles as $role)
    <div class="col-md-12 b-all mt-2">
        <div class="row bg-dark p-3 justify-content-center align-items-center">
            ...
            <div class="col-md-4">
                <button class="btn btn-default btn-sm btn-rounded pull-right" onclick="toggleRolePermission('#role-permission-{{ $role->id }}')" data-role-id="{{ $role->id }}"><i class="fa fa-key"></i> @lang('modules.rolePermission.permissions')</button>
            </div>
        </div>
        <div class="row">
...
@push('footer-js')
    <script>
        const create_form = `@include('superadmin.role-permission.create_form')`;
        ...
        function togglePermission(options, ele) {
            ...
            $.easyAjax({
                url: '{{ route('superadmin.role-permission.store') }}',
                type: 'POST',
                data: options,
            })
        }
        ...
        function toggleAllPermissions(options, ele) {
            ...
            $.easyAjax({
                url: '{{ route('superadmin.role-permission.toggleAllPermissions') }}',
                type: 'POST',
                data: options,
        ...
        $('body').on('click', '#addRole', function() {
            const url = '{{ route('superadmin.role-permission.create') }}';
        ...
        $('body').on('click', '#save-create-role', function () {
            var url = '{{ route("superadmin.role-permission.addRole")}}';
        ...
        $('body').on('click', '.edit-role', function () {
            ...
            var url = '{{ route("superadmin.role-permission.edit", ":id")}}';
        ...
        $('body').on('click', '#save-edit-role', function () {
            ...
            var url = '{{ route("superadmin.role-permission.update", ":id")}}';
            url = url.replace(':id', id);
        ...
        $('body').on('click', '.delete-role', function () {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var url = '{{ route("superadmin.role-permission.destroy", ":id")}}';
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.show-members', function() {
            const id = $(this).data('role-id');
            let url = '{{ route('superadmin.role-permission.show', ':id') }}';
            url = url.replace(':id', id);

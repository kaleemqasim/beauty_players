...
<script>
    ...
    function renderSelect() {
        $.easyAjax({
            url: '{{ route('superadmin.role-permission.getMembersToAdd', $id) }}',
            type: 'GET',
    ...
    roleMemberTable = $('#roleMemberTable').dataTable({
        ...
        pageLength: 3,
        lengthChange: false,
        ajax: '{!! route('superadmin.role-permission.getMembers', ['role_id' => $id]) !!}',
    ...
    $('body').on('click', '#save-add-member', function() {
        $.easyAjax({
            url: '{{ route('superadmin.role-permission.addMembers', ['role_id' => $id]) }}',
            type: 'POST',
    ...
    $('body').on('click', '.delete-member', function () {
        ...
        swal({
            ...
            if (willDelete) {
                let url = '{{ route('superadmin.role-permission.removeMember') }}';
